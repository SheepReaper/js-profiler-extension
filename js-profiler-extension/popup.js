...
document.getElementById('show').onclick = async () => {
  ...
  chrome.tabs.sendMessage(tab.id, { type: 'getTimings' }, (timings) => {
    ...
    // Render table (Bulma)
    let html = '<table class="table is-striped is-hoverable is-fullwidth" style="font-size:12px"><thead><tr><th>Function</th><th>Count</th><th>Min (ms)</th><th>Max (ms)</th><th>Avg (ms)</th></tr></thead><tbody>';
    for (const s of stats) {
      html += `<tr><td>${s.name}</td><td>${s.count}</td><td>${s.min}</td><td>${s.max}</td><td>${s.avg}</td></tr>`;
    }
    html += '</tbody></table>';
    // Simple bar chart for call frequency (Bulma)
    const maxCount = Math.max(...stats.map(s => s.count));
    html += '<h4 class="title is-6 mt-4 mb-2">Call Frequency</h4>';
    for (const s of stats) {
      const bar = '<span class="bar" style="width:' + (s.count / maxCount * 100) + '%"></span>';
      html += `<div style="font-size:11px" class="mb-1">${s.name} <span class="tag is-light is-info ml-1">${s.count}</span> ${bar}</div>`;
    }
    // Timeline visualization (Bulma heading)
    ...
    html += '<h4 class="title is-6 mt-4 mb-2">Timeline</h4>';
